<!--
 * @Autor: fanmm
 * @Date: 2020-12-08 09:46:01
 * @LastEditors: fanmm
 * @LastEditTime: 2020-12-08 15:31:38
-->
<template>
  <div class="turn" ref="turn"  :style="{width: width*2+'px', height: height+'px',border:'1px solid red'}">
    <template v-for="(item,index) in data">
      <turn-page  v-if="index == backPage || index == turnPage" v-bind="$attrs" :index="index" :width="width" :item="item" :height="height" :length="data.length" :active="index == turnPage && turnActive" :styles="index === turnPage ? styles : defaultStyles">
      </turn-page>
    </template>
  </div>
</template>
...
<script>
import TurnPage from "../components/turnPage/index.vue";
