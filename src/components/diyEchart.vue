/*
 * @Author: fanmm 
 * @Date: 2020-10-10 11:08:35 
 * @Last Modified by: fanmm
 * @Last Modified time: 2020-10-10 13:57:29
 */
<template>
...
export default {
  ...
  methods: {
    ...
    renderItem(params, api) {
      ...
      var rectShape1 = this.$echarts.graphic.clipRectByRect(
        {
          //矩形的位置
          x: planStartDate[0],
          y: planStartDate[1],
      ...
      if (api.value(3) !== "") {
        ...
        //实际开始日期(毫秒值)
        var practiceStartDate_millisecond = +this.$echarts.number.parseDate(
          api.value(3)
        );
        //当前日期(毫秒值)
        var nowDate_millisecond = +this.$echarts.number.parseDate(this.nowDateStr);
        //如果项目进度未完成或刚好完成。
        ...
        //使用graphic图形元素组件，绘制矩形
        //clipRectByRect方法，在绘制矩形时，如果矩形大小超出了当前坐标系的包围盒，则裁剪这个矩形
        rectShape2 = this.$echarts.graphic.clipRectByRect(
          {
            //矩形的位置
    ...
    setOption(){
        ...
        tooltip: {
            ...
            formatter: function (params) {//params为当前点击图形元素的数据信息的对象
                ...
                var projectCycle_millisecond = +(_this.$echarts.number.parseDate(params.value[2])) - +(_this.$echarts.number.parseDate(params.value[1]));
                //项目周期(天数)
                var projectCycle_days = projectCycle_millisecond / _this.dayTime;
                //当前进度(百分比)
                ...
                if (practiceStartDate !== '') {//如果实际开始日期不为空，说明项目已开始
                    ...
                    var currentProgress_mill = +_this.$echarts.number.parseDate(_this.nowDateStr) - +_this.$echarts.number.parseDate(params.value[3]);
                ...
                if (currentProgress_percentage == 100) {//如果项目进度已完成或项目已结束
                    ...
                    var practiceEndDate_millisecond = +_this.$echarts.number.parseDate(practiceStartDate) + projectCycle_millisecond;
                    //实际结束时间(日期格式)
                    practiceEndDate = _this.$echarts.format.formatTime('yyyy-MM-dd', practiceEndDate_millisecond);
